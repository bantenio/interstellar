...
public class MediaType extends MimeType implements Serializable {
    ...
    private static final long serialVersionUID = -9229337799739907L;
    ...
    protected void checkParameters(String parameter, String value) {
        ...
        if (PARAM_QUALITY_FACTOR.equals(parameter)) {
            ...
            Assert.isTrue(d >= 0D && d <= 1D,
                    () -> new IllegalArgumentException("Invalid quality value \"" + unquotedValue + "\": should be between 0.0 and 1.0"));
        }
    }
    ...
    public static List<MediaType> parseMediaTypes(String mediaTypes) {
        if (CharSequenceUtil.isNotEmpty(mediaTypes)) {
     ...
     * @since 4.3.2
     */
    public static List<MediaType> parseMediaTypes(Collection<String> mediaTypes) {
        if (CollectionUtil.isEmpty(mediaTypes)) {
            return Collections.emptyList();
        } else if (mediaTypes.size() == 1) {
            return parseMediaTypes(mediaTypes.iterator().next());
        } else {
